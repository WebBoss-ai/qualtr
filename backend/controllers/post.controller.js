    ...
    const postsWithMediaUrls = await Promise.all(
      shuffledPosts.map(async (post) => {
        ...
        return {
          ...
          profileLink: `/founder-profile/${post.author._id}`, // Link to author's profile
        };
      })
    );
...
const CATEGORY_ENUM = {
  STARTUP_ESSENTIALS: 'Startup Essentials',
  MARKETING_BRANDING: 'Marketing & Branding',
  LEGAL_COMPLIANCE: 'Legal & Compliance',
  FINANCE_INVESTMENT: 'Finance & Investment',
  SALES_CUSTOMER_ACQUISITION: 'Sales & Customer Acquisition',
  TECHNOLOGY_TOOLS: 'Technology & Tools',
  INSPIRATIONS: 'Inspirations',
};

// Reusable function to fetch posts by category
const getPostsByCategory = async (category, req, res) => {
  try {
    ...
    let loggedInUserProfilePhoto = null;
    if (digitalMarketer.profile?.profilePhoto) {
      loggedInUserProfilePhoto = await getObjectURL(digitalMarketer.profile.profilePhoto);
    }

    const postsWithMediaAndAuthorData = await Promise.all(
      posts.map(async (post) => {
        // Process author profile photo
        if (post.author?.profile?.profilePhoto) {
          const profilePhotoURL = await getObjectURL(post.author.profile.profilePhoto);
          post.author.profile.profilePhoto = profilePhotoURL;
        }
    ...
    console.error(`Error retrieving posts for category ${category}:`, error);
    return res.status(500).json({
      message: 'Internal server error.',
      success: false,
    });
...
export const getStartupEssentialsPosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.STARTUP_ESSENTIALS, req, res);

export const getMarketingBrandingPosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.MARKETING_BRANDING, req, res);

export const getLegalCompliancePosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.LEGAL_COMPLIANCE, req, res);

export const getFinanceInvestmentPosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.FINANCE_INVESTMENT, req, res);

export const getSalesCustomerAcquisitionPosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.SALES_CUSTOMER_ACQUISITION, req, res);

export const getTechnologyToolsPosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.TECHNOLOGY_TOOLS, req, res);

export const getInspirationsPosts = (req, res) =>
  getPostsByCategory(CATEGORY_ENUM.INSPIRATIONS, req, res);
